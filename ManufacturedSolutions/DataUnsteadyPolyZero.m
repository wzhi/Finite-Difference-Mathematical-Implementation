function [exact, src, bc] = DataUnsteadyPolyZero(kappa, rho, xspan, yspan)
%DATAUNSTEADYPOLYZERO  Polynomial data for unsteady PDE with vanishing boundary values.
% [U,SRC,BC]=DATAUNSTEADYPOLYZERO(KAPPA,RHO) returns the solution U to the
...
if ~license('test', 'symbolic_toolbox') || verLessThan('symbolic', '4.0.0')
  ...
  exact = @DataUnsteadyPolyZeroExactSolution;
  src = @DataUnsteadyPolyZeroDomainSource;
  bc = @DataUnsteadyPolyZeroNeumannSource;
  return
end
...
x1 = sym('x1');
x2 = sym('x2');
u = cos(t)*prod(x1 - xspan)*prod(x2 - yspan);

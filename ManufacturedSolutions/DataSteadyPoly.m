function [exact, src, bc] = DataSteadyPoly(kappa, rho)
%DATASTEADYPOLY  Polynomial data for steady PDE.
% [U,SRC,BC]=DATASTEADYPOLY(KAPPA,RHO) returns the solution U to the
% boundary value problem
%
%  DIV(KAPPA*GRAD(U)) + RHO*U = SRC(T,X1,X2)  on the interior
%        [N1, N2]*GRAD(U) = BC(T,X1,X2,N1,N2) on the boundary
%
%  * KAPPA may be a scalar or 2x2 matrix of diffusion coefficients
...
if ~license('test', 'symbolic_toolbox') || verLessThan('symbolic', '4.0.0')
  ...
  assert(isequal(kappa, [1 3; 2 4]))
  assert(isequal(rho, 2))
  % NB: STR2FUNC unavailable before R2009a
  exact = @DataSteadyPolyExactSolution;
  src = @DataSteadyPolyDomainSource;
  bc = @DataSteadyPolyNeumannSource;
  return
end
